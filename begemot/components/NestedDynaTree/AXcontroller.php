...
class AXcontroller extends CController {
    ...
    private function getModel() {
        ...
        if (isset($_POST['model'])) {
            ...
            if (!($var->roots()))
                throw new CDbException(Yii::t('tree', 'Model `' . $_POST['model'] . '` have to be an instance of NestedTreeActiveRecord, or have NestedSet behavior.'));
            else
                return $_POST['model'];
        }
        return false;
    ...
    public function actionLoad() {
        if (($modelClass = $this->getModel()) !== false) {
            ...
            echo json_encode($model->getTree($url));
        }
        else
            echo 0;
    }
    ...
    public function actionInsert() {
        ...
        if (($modelClass = $this->getModel()) !== false) {
            ...
            $target = $model->findByPk($_POST['source']);
            $node = new $modelClass;
            
            try {
                if($target->isRoot())$status = $node->appendTo($target, false);
                else
                switch ($_POST['mode']) {
                    case "after":
                ...
                echo json_encode(array('status' => ($status === true) ? 1 : 0, 'node' => $target->primaryKey, 'title' => $node->nodeTitle, 'key' => $node->primaryKey));
            } catch (Exception $e) {
    ...
    public function actionMove() {
        ...
        if (($modelClass = $this->getModel()) !== false) {
            ...
            $node = $model->findByPk($_POST['source']);
            $target = $model->findByPk($_POST['target']);
            try {
                switch ($_POST['mode']) {
                    case "before":
                ...
                echo json_encode(array('status' => ($status) ? 1 : $status, 'sourceNode' => $node->primaryKey, 'node' => $target->primaryKey, 'mode' => $_POST['mode']));
            } catch (Exception $e) {
                echo json_encode(array('status' => $e->getMessage()));
            }
        }
    ...
    public function actionDelete() {
        ...
        if (($modelClass = $this->getModel()) !== false) {
            ...
            try {
                ...
                echo json_encode(array('status' => ($status === true) ? 1 : 0, 'node' => $target->primaryKey, 'key' => $prevnode->primaryKey));
            } catch (Exception $e) {
                echo json_encode(array('status' => $e->getMessage()));
            }
        }
